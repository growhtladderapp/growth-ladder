  ...
  if (!isPro) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] p-8 text-center animate-fade-in">
         <div className="bg-zinc-900 p-8 rounded-full border border-zinc-800 shadow-2xl mb-6">
            <SportyRobotIcon size={64} className="text-slate-600" />
         </div>
         <h2 className="text-2xl font-black text-white mb-2 uppercase tracking-tighter italic">Acceso Restringido</h2>
         <p className="text-slate-400 mb-8 max-w-xs text-sm">
           La consultoría de rendimiento de élite está reservada exclusivamente para usuarios de nivel PRO.
         </p>
         <div className="flex items-center gap-2 px-4 py-2 bg-emerald-900/20 border border-emerald-500/20 rounded-lg text-emerald-500 text-xs font-bold uppercase tracking-wider">
            <ShieldCheck size={14} /> Protocolo Pro Requerido
         </div>
      </div>
    );
  }
        ...
        {messages.map((msg, idx) => {
          ...
          return (
            <div key={idx} className={`flex ${isModel ? 'justify-start' : 'justify-end'}`}>
               <div className={`max-w-[85%] rounded-2xl p-4 shadow-xl relative ${
                 isModel 
